...
function ReportCard({
  ...
  const formatPeriod = (report: ReportV2) => {
  ...
  const formatSubPeriod = (report: ReportV2) => {
    const endDate = new Date(report.endTime);
  ...
  const fortuneScore = parseInt(report?.reportContent?.fortuneScore ?? 100);
...
function DashboardContent() {
  ...
  >("ALL");
  const [reports, setReports] = useState<ReportV2[]>([]);
  ...
  const handleReportClick = (report: ReportV2) => {
  ...
  return (
    <div className="min-h-screen px-4 py-4 md:py-8 max-w-7xl mx-auto">
      ...
      <FortuneCookieAnimation
        ...
        fortuneScore={75}
        fortuneText={
          selectedReport?.reportContent?.general || "Your destiny awaits..."
        }
        luckyNumbers={[11, 13]}
      />
    </div>
