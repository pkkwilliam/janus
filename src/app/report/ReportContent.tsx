...
interface CachedBornYear {
  bornYear: number;
  reading: string;
}

interface CachedZodiacFortune {
  zodiac: string;
  ...
  health: string;
  relationship: string;
  monthly: CachedMonthlyReading[];
  bornYear: CachedBornYear[];
}
...
function ReportContent() {
  ...
  // Get zodiac using Lunar calendar calculation
  const getZodiacForDate = (date: Date) => {
    return getZodiac(date);
  };

  // Get current content (original or translated, or cached)
  const getCurrentContent = () => {
    // For cached mode, return cached content
    if (cachedFortune) {
  ...
  const getCachedContent = () => {
    ...
    return {
      ...
      career: cachedFortune.career,
      financial: cachedFortune.financial,
      health: cachedFortune.health,
      relationship: cachedFortune.relationship,
      bornYear: bornYearReading,
      monthly: transformedMonthly,
      zodiacChinese: cachedFortune.zodiacChinese,
    };
  };

  const getCurrentContentV2 = () => {
    // Use cached fortune if available, otherwise use report content
    const cachedContent = getCachedContent();
    const reportContent = cachedContent || getCurrentContent();
    ...
    return [
      ...
      },
      {
        ...
        show: !!reportContent?.relationship,
      },
      {
  ...
  useEffect(() => {
    if (birthDate) {
      // Load from cached report for free reading
      loadCachedFortune();
  ...
  // Show error state
  if (reportError || (!report && !cachedFortune)) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <div className="flex gap-3 justify-center">
            ...
            <button
              onClick={birthDate ? loadCachedFortune : loadReport}
  ...
  return (
    <div className="min-h-screen px-4 py-8 max-w-4xl mx-auto">
      ...
      {/* Only show lucky elements sections for non-cached reports or if data exists */}
      {!isCachedMode && (
        <>
      ...
      {/* Born Year - Chinese Zodiac Display - only show if data exists */}
      {(reportContent as any)?.zodiac && (
        <BornYearSection
          zodiac={(reportContent as any).zodiac}
